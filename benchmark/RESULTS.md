...
### 更正：`CacheItemModels` 的收益从未到达帧时间

上一节写"CPU 少了 301us，在这台机器上不转化为帧率，因为 GPU 更长"。**这个解释证据不足，撤回。**

给 harness 加了 `-WindowWidth/-WindowHeight` 想造一个 CPU 瓶颈来验证。第一次尝试无效 ——
1.8.9 里 `options.txt` 的 `overrideWidth` 压过 `--width` 启动参数，而 `options.benchmark.txt`
钉着 1280，结果 JSON 老实报了 1280×720。改走 options 之后：

| GPU `frameTotal` | 1280×720 | 640×360 |
| --- | ---: | ---: |
| off | 1791 us | 1863 us |
| on | 1721 us | 1696 us |

**像素少四分之三，GPU 时间几乎没变。** 这个场景不是填充率受限，是**几何提交／draw call 受限**
（103 实体 × 6 个模型），所以缩窗口这个办法对它不成立，CPU 瓶颈造不出来。

四次测量（1280×720 三次 + 640×360 一次）：`entityLayers` 每次都 −46%、负载计数每次都一致，
**帧时间和帧率没有一次出现一致方向的变化**，`trace.py` 每次都判 DIRECTION FLIPS。

于是只剩两种读法，而且分不开：**工作确实没了但机器显示不出来**，或者**工作只是被推迟、profiler
把等待记到了别处**。驱动受限时，section 计时无法区分这两者 —— 我先前的表述默认了第一种。

改为默认关。代码保留：它是正确的、画面逐位一致、在真正 CPU 受限的机器上仍可能兑现，
但这台机器给不出那个证据。要定案需要一台 CPU 受限的机器，或者三轮系列把噪声压下去。

## `EntityCulling`：本轮唯一到达帧率的实体优化

### 先说它为什么之前测不出来

在 `replay-pit` 上跑，`cullProbesIssued = 0` —— **一次探针都没发**。原因是
`EntityCullingMinEntities` 默认 24，而 pit 每帧只渲染 21.8 个实体，低于阈值就整个跳过。
那一轮的 +7.7% 帧率与这个功能无关（`terrainDrawCalls` 282.4 vs 220.6，是负载差）。
**选错场景。**

换到 `armor-dense-quick`：103 个实体过阈值，场景里那堵 `/fill -20 7 12 20 20 12` 的石墙
把 z 15..25 的两圈挡在后面，约一半可剔。

### 结果

| | off | on |
| --- | ---: | ---: |
| `cullProbesIssued` | 0 | **21840** |
| `cullProbesOccluded` | 0 | **10920（恰好一半）** |
| `entitiesCulled`/帧 | 0 | **39.9 / 108.3（37%）** |
| **avg fps** | 334.9 | **408.4（+22.0%）** |
| p50 帧时间 | 2.851 ms | **2.345 ms（−17.7%）** |
| CPU `frameTotal` | 2178 us | 1743 us |
| **GPU `frameTotal`** | **1821 us** | **1505 us（−17%）** |

trace.py 十段方向全部一致（+0.42~+0.57ms）。像素闸门 0.026% / 0.064% / 0.053%，
与空对照（0.039%）同量级 —— 被剔掉的实体确实本来就看不见。

### 为什么它成立而 `CacheItemModels` 不成立

**GPU 时间降了 17%。** 两个场景都是几何提交受限（缩到四分之一像素 GPU 时间不变），所以
让 CPU **更快地把工作交出去**买不到东西，**根本不交**才买得到。

这是本轮最有用的一条结论：在这台机器上，只降 CPU 提交成本的优化无法兑现；减少提交量的才能。

### 改为默认开

`CullPlayers` 仍默认关 —— 遮挡测试比世界晚一帧，漏画一个玩家和漏画一个盔甲架是两种性质的 bug，
而默认配置把前者排除在外。`EntityCullingMinEntities` 把它挡在付不起的场景之外。

## `EntityCulling`：把实体数量阈值换成遮挡率状态机

### 阈值错在哪

`EntityCullingMinEntities = 24` 是唯一的守门条件，而**实体数量根本不预测遮挡**：一百个实体在
开阔地互相不遮，二十五个在走廊里大半被遮。后果是真实的 Hypixel 战斗（每帧 21.8 个实体）
**从未越过阈值**，这个功能在针对它的每一次测量里都全程休眠（`cullProbesIssued = 0`）。

它挡的其实不是查询成本，是**遍历成本**：

```java
// 只为发现实体数量从来不够，就对每个已加载实体跑六个平面的视锥测试 —
// 在这个功能全程休眠的大厅场景里，代价是 6.9% 的帧率。
```

原有的 `backoff` 只乘在**每实体重探间隔**上（`reprobeMillis * backoff`），遍历照样每帧跑，
所以它减不掉那 6.9%。

### 改成什么

两状态机，控制信号是探针本来就在算的遮挡率（`RATE_WINDOW = 256` 个判定，5%/10% 迟滞）：
...
节流的是**整个 sweep**（含遍历），不只是查询间隔 —— 300fps 下侦察态每 150 帧才遍历一次，
那 6.9% 变成约 0.05%，于是数量阈值不再需要，默认改为 0。

**"只探测不隐藏"不是保守，是让长周期安全的前提。** 半秒前的判定不能拿来隐藏实体（相机早就
动了），所以探得稀疏就必须不作为，要作为就必须探得勤 —— 两者绑在一起才成立。切换到剔除态时
把侦察期的判定全部作废重探（`discard || !wasInFrustum()`）。

在线测帧时间收益不可行 —— 本项目用**受控 benchmark** 都反复把噪声读成效果。遮挡率是无噪声
的直接计数，是能用的代理。

### 结果（`armor-dense-quick`，状态机版）

| | off | on |
| --- | ---: | ---: |
| `cullProbesOccluded` | 0 | 10200 / 20400 |
| `entitiesCulled`/帧 | 0 | 37.6 |
| avg fps | 267.9 | **311.7（+16.4%）** |
| GPU `frameTotal` | 2612 us | **2124 us（−18.7%）** |

像素 0.001% / 0.027% / 0.013%。这个场景本来就过旧阈值，所以它只证明状态机没坏；
改动的价值要看 pit —— 那才是旧阈值把功能完全挡在门外的场景。

### 结果（`replay-pit`，旧阈值把功能完全挡住的场景）

| | off | on |
| --- | ---: | ---: |
| `cullProbesIssued` | 0 | **11** |
| 遮挡率 | — | **72.7%** |
| `entitiesCulled`/帧 | 0 | **0.00** |
| fps | 352.9 | 361.7（trace 判定 DIRECTION FLIPS，噪声） |

状态机确实开始探测了（旧实现在这里是 0 次），但 11 秒只发了 11 次探针，而每帧渲染 21.8 个实体。

**原因不是阈值 —— 是 `CullPlayers` 默认关，而 pit 里的实体几乎全是玩家。** 剩下符合条件的只有
零星掉落物。72.7% 的遮挡率说明机制本身没问题，是符合条件的实体太少。

**旧的 24 阈值一直在掩盖第二个原因**：就算把阈值拿掉，这个功能在 PvP 场景下依然几乎无事可做。
对一个 PvP 客户端来说，这比阈值本身重要得多。

### 遮挡率单独用不够：`CullPlayers` 逼出来的修正

把数量阈值整个删掉是错的。`replay-pit` 开 `CullPlayers` 后遮挡率 **65.5%** —— 远超 10% 的上界，
状态机据此进入剔除态，然后：

| `replay-pit` | CullPlayers off | on |
| --- | ---: | ---: |
| `cullProbesIssued` | 11 | 995 |
| `entitiesCulled`/帧 | 0 | 2.36 |
| **fps** | **335.8** | **320.7（−4.5%，十段九段一致）** |

**每帧藏住 2.36 个实体，付不起一次 sweep 二十次探针。**

两个信号回答的是不同问题：**遮挡率说"有没有东西被挡住"，实体数量说"挡住的值不值得省"。**
旧的 24 阈值哪个都没答对（它数的实体可能全在明处），但它守的正是后者 —— 删掉它就把这个 case
放进来了。

正确的信号是两者的乘积，也就是**每次 sweep 实际藏住的绝对数量**（`CULL_FLOOR = 8`）：
pit 2.36 → 不值，armour-dense 37.6 → 很值。

### 加地板之后

| | pit（CullPlayers on） | armour-dense |
| --- | --- | --- |
| 状态 | 侦察（`culled/f = 0.00`，探针 995 → **70**） | 剔除（`culled/f = 38.2`） |
| 帧率 | 中性（**−4.5% 消失**） | **339.7 → 422.2（+24.3%）** |
| GPU | — | 1744 → 1419 us（−18.6%） |

trace 在 armour-dense 上十段方向一致，像素三张全过。

**`CullPlayers` 的结论**：真实 PvP 录像上 −4.5%。它默认关不再只是"有名牌风险所以保守"，
现在有实测支撑 —— 尽管加地板后它已经不会主动伤害，仍然没有理由打开。

## 仪器：GPU 计时现在报覆盖率，以及它揭穿了我的假设

各 section 的 GPU 数字之前无法使用 —— 同场景不同轮之间摆动很大，`entities` 和 `terrain` 报 0，
加起来只有整帧的一小部分。我的假设是取不到查询结果时被记成 0，混进样本数组后把中位数压到 0。

改成取不到记 −1、百分位只在有效样本上算，并输出 `samples` 和 `coverage`。结果：

| section | CPU | GPU p50 | coverage |
| --- | ---: | ---: | ---: |
| frameTotal | 828 | 1093 | 100% |
| hud | 204 | 242 | 100% |
| sky | 18 | 79 | 100% |
| terrain | 156 | 17 | 100% |
| **entities** | 316 | **0.0** | **100%** |
| textureAnim | 0 | 36 | **4.4%** |

**覆盖率 100%，所以那些 0 是量出来的，不是缺失的 —— 假设错了。**

修复仍然值得：现在能把"真的是 0"和"没测到"分开。`textureAnim` 的 4.4% 覆盖率是真的低采样，
以前它和 `entities` 在输出里完全同形。**一个 GPU 数字脱离采样数就没有意义。**

**未解决**：各 section 合计 384us，整帧 GPU 1093us —— **65% 落在所有命名区间之外**；
而 `entities` 每帧画 130 个模型却量到约 0。`glQueryCounter(GL_TIMESTAMP)` 是流内时间戳，
end − start 本应就是那段命令的 GPU 时间。不在这里推测原因，下一步是加 bracket 把缺口逼出来。

## getRGB 绕过：已完成，剩下的部分不值得做

`FastTextureUpload`（默认开）已经在做这件事 —— 去掉 4M int 的暂存分配，并在能直读时绕开
`getRGB`。实测覆盖率：

| | 整轮 | 测量窗口 |
| --- | ---: | ---: |
| `textureUploads` | 177 | 10 |
| `textureUploadsDirect` | **133（75%）** | 8 |

### 剩下 25% 是什么

把诊断埋在真正的回落点之后，只剩两个类型：`TYPE_BYTE_INDEXED`（16×16）和
`TYPE_BYTE_BINARY`（256×256）。`TYPE_4BYTE_ABGR` 和 `TYPE_3BYTE_BGR` **已经被字节路径接住**。

**第一次诊断埋错了地方**：放在 `directPixels()` 里，它对任何非 `TYPE_INT_ARGB` 都返回 null，
于是把随后被字节路径成功处理的类型也报了出来 —— 差一点据此去实现一条已经存在的路径。

### 为什么不做

一张 16×16 的索引图是 256 个像素；256×256 的二值图约 65K 像素走一次 `IndexColorModel`，
估算 5ms 量级，**一次，在加载期**。相对已有的 75% 覆盖，剩余部分不值得那份调色板解包代码。

### 真正的缺口是没法测

**这个优化从来没被量过收益，包括已经默认开的那 75%。** 纹理上传发生在加载期，而 benchmark
的测量窗口是稳态游戏 —— 测量窗口里只有 10 次上传、16 万像素，对帧时间没有意义。
路线图 §4.6 早就写明"不把资源加载优化宣传为游戏内平均 FPS 优化"，但也就没人给它一个数字。

要给它数字，需要 harness 支持**加载阶段计时**（启动到 settle 的耗时、资源包切换耗时、
上传累计耗时）。那是独立的一项工作，不是 getRGB 本身。

## 加载阶段计时，以及 `FastTextureUpload` 上线以来的第一个数字

这个 harness 报告的一切都是稳态：测量窗口在世界起来并预热之后才打开。纹理上传、图集拼接、
mipmap、资源包切换**从来不在任何被测区间里** —— 加载不是由帧构成的。所以 `FastTextureUpload`
默认开着，却没有一个数字。

新增：`textureUploadNanos` / `textureUploadDirectNanos` 计数器（计时起点在**启用判断之前**，
否则关掉时什么都不计、没有对照组；原版路径由 `@At("RETURN")` 单独计时，与 `ci.cancel()` 的
快速路径互斥），以及结果里的 `phaseMillis`（loadWorld / loadReplay / settle / warmup /
discard / measure 的墙钟时长）。

### 结果（`replay-pit` 整轮，`-DiscardPasses 0`，因为冷启动就是被测对象）

| | off | on |
| --- | ---: | ---: |
| 上传次数 | 181 | 181 |
| 直读命中 | 4（2%） | 133（73%） |
| 上传像素 | 3,421,600 | 3,421,600 |
| **上传总耗时** | **160.2 ms** | **46.2 ms** |

**−114ms，−71%。** 次数与像素完全相同。

### 更正：上一节"剩下 25% 不值得做"是错的

拆开 on 的 46.2ms：直读 133 次共 **7.8ms**，回落 48 次共 **38.4ms** —— 回落占 27% 的次数、
**83% 的剩余耗时**，每次 0.8ms。

上一节估的是"5ms 量级"，**实际 38ms，差了近八倍**。那是个没有测量支撑的估算。
把剩余的 `TYPE_BYTE_INDEXED` 和 `TYPE_BYTE_BINARY` 接进直读路径，还能省约 38ms 加载时间。

### 已知缺口

`phaseMillis` 从 `BenchRunner` 启动算起，**不含客户端 boot**（JVM、Forge、资源加载）——
而那 114ms 大部分就发生在那之前。`countersTotal` 覆盖全程所以上面的数字有效，但"加载总耗时"
这个问题还没被完整回答。

### 索引调色板直读路径

`TYPE_BYTE_INDEXED` 和 `TYPE_BYTE_BINARY` 的像素本来就是调色板索引，而 `getRGB` 每个像素
查一次 `IndexColorModel`。把调色板一次性解析成 `int[]`，之后每像素一次数组读。

| `replay-pit` 整轮 | 加索引路径前 | 加索引路径后 |
| --- | ---: | ---: |
| 直读覆盖 | 133/181（73%） | **177/181（98%）** |
| 真回落 | 48 次 / 38.4 ms | **0 次**（剩 4 次是模块启用前的上传，两变体相同） |
| 上传总耗时 | 46.2 ms | 36.5 ms |

**像素闸门通过**（0.028% / 0.058% / 0.017%，空对照 0.039% 同量级）—— 调色板解码正确。

刻意保守两处：不直接解 `TYPE_BYTE_BINARY` 的位打包（每字节 8 像素、布局随位深变化，算错就是
全游戏贴图损坏），改用 `raster.getSample()` 逐样本读；不透明调色板补 `0xFF000000`，否则整张
图全透明 —— 而那种错误在 benchmark 里表现为帧率变好。

### 三次估算，三次错

| | 估计 | 实测 |
| --- | ---: | ---: |
| 剩余回落的成本 | ~5 ms（"不值得做"） | **38.4 ms** |
| 索引路径能省多少 | ~38 ms | **~10 ms** |

第一次低估八倍，第二次高估近四倍，**方向还相反**。第二次偏差的原因是我为安全选了逐样本读，
省掉颜色模型查找（贵的部分）但保留了栅格访问开销 —— 这个取舍仍然值得。

**这个量级上估算没有价值，只有计数器和实测有。**

### 加载期测量的结构性困难

两轮的 `off` 基线是 160.2 ms 和 182.7 ms —— **加载耗时轮间方差 14%**，而 46.2 → 36.5 跨了两次
运行，落在噪声里。稳态测量可以在一轮内交错对照，**加载只发生一次**，做不到。要坐实幅度需要多轮
取中位数，而每轮都得跑完整加载。所以覆盖率（计数）是这一节唯一硬的结论。

## 这个 campaign 横跨两块 GPU，§9.4 的部分否决因此失效

| GPU | 运行数 | 时间段 |
| --- | ---: | --- |
| NVIDIA RTX 5060 Laptop（独显） | 391 | 07-26 ~ 07-28 |
| **AMD Radeon 610M（核显）** | 229 | 07-27 ~ 08-01 |

**7 月 28 日之前的结论全在独显上测得，之后全在核显上。** 两者瓶颈不同 —— 核显是几何提交受限。

Render Regions 的否决理由是"独显实测每帧只有 122.6 次区块 draw call，且早先已证明砍掉 333 次
draw call/帧毫无可测量影响"。**当前机器实测 444.4 次/帧**，且帧时间就卡在提交上。

### 地形天花板探针（`halfTerrainDraws`，隔一个取消一个）

| `replay-pit` | full | half |
| --- | ---: | ---: |
| avg fps | 470.2 | **592.2（+26%）** |
| p50 帧时间 | 1.991 ms | **1.522 ms（−23.6%）** |
| **GPU `frameTotal`** | **1049 us** | **672 us（−36%）** |
| CPU `frameTotal` | 799 | 763（几乎不动） |

**地形渲染在这台机器上值 26% 帧率，几乎全在 GPU 侧。** 旧否决在这里不成立。

**但这不证明 Render Regions 有用。** 探针同时砍掉 draw call 和三角形，分不出省的是哪一个；
Render Regions 保留全部三角形只合并绘制，若成本在三角形吞吐上则毫无用处。**这一步只定了上界。**

### `trace.py` 的判据改了两次，第一次改错

原判据要求十段方向全部一致，把 +26% 判成 "not one effect"。改成 7/10 多数决之后，
**它同时放行了一个已知无效的配对**（层级探针，两变体负载差 16%）—— 两者都是 7/10，
符号计数没有分辨力。

改用**效应大小相对于分段离散度**（中位数 ÷ 平均绝对偏差）。四个已知答案的配对全部判对：

| 配对 | 已知 | 比值 | 判定 |
| --- | --- | ---: | --- |
| 地形天花板 | 真效应 | 1.77 | clear |
...
## 任务 22：地形的成本在三角形，不在 draw call —— Render Regions 关闭

`halfTerrainDraws` 同时砍掉绘制次数和三角形，给出 +26%，只能定上界。分离办法：把每个区块的
一次绘制拆成两次覆盖同一段缓冲（切点取 4 的倍数，否则 `GL_QUADS` 被切碎）——
**三角形完全不变，draw call 翻倍**。

| `replay-pit` | single | split |
| --- | ---: | ---: |
| `terrainDrawCalls`/帧 | 284.2 | **551.5** |
| avg fps | 453.4 | 452.0 |
| GPU `frameTotal` | 1088 us | 1062 us |
| trace 判定 | — | **NOT ONE EFFECT**（比值 0.14） |

**把地形 draw call 翻倍，代价为零。** 像素三张全过（0.002% / 0.027% / 0.020%），
draws 确认从 403.8 翻到 808.6 —— 排除了"探针在画垃圾所以不花时间"。

所以那 26% **全部来自三角形**。Render Regions 保留全部三角形、只合并绘制，**拿不到任何东西**。
旧的否决结论是对的，但依据（独显、122.6 次 draw call）一直不适用于这台机器；现在有了适用的依据。
...
原表述"减少提交量能兑现，减少提交成本不能"**太宽**。正确的是：**能兑现的是减少几何量，
不是减少 draw call 数量。** `EntityCulling` 有效是因为它连同三角形一起删掉实体，
不是因为它减少了绘制次数。这也解释了 `CacheItemModels` 为什么拿不到收益 —— 它一个三角形都没少。

## 任务 23：加载期定价 —— §4.6 的优先级排反了

`phaseMillis` 加上四个计数器（上传、mipmap、解码、图集），第一次把加载期拆开：
...
| ├ **拼接 + 精灵装配** | **510.7** | — | **67%** |
| ├ PNG 解码 | 118.4 | 732 | 15% |
| ├ mipmap 生成 | 100.2 | 888 | 13% |
| └ 纹理上传 | 36.4 | 181 | **5%** |

**上传是四者里最小的一项，而它是 §4.6 里唯一被实现的。** 拼接器比它大 14 倍，从没被测过 ——
优先级完全排反了，而排它的时候没有任何一项有数字。

### 建议：记下数字，不要动手

这 765ms 是**一次性**的，发生在客户端启动和资源包切换时，游戏过程中不复现。就算把拼接器优化
掉一半也只省 0.4 秒一次。§4.6 自己写着"不把资源加载优化宣传为游戏内平均 FPS 优化"——
这些数字支持那句话，而不是推翻它。

`FastTextureUpload` 仍然值得保留（160–183ms → 36.5ms，见前节），但它买到的是加载期的
120ms，不是帧率。**如果将来资源包切换卡顿成为实际抱怨，拼接器的 510ms 才是第一个该看的地方。**
