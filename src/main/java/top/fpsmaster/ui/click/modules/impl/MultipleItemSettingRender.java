...
public class MultipleItemSettingRender extends SettingRender<MultipleItemSetting> {
    ...
    public void render(float x, float y, float width, float height, float mouseX, float mouseY, boolean custom) {
        ...
        Rects.rounded(x + xOffset, y + FPSMaster.fontManager.s16.getHeight() + 5, itemWidth + padding, this.height - 7, 3, new Color(80, 80, 80, 160).getRGB());
        ...
        for (ItemStack itemStack : setting.getValue()) {
            Rects.rounded(x + xOffset + padding, y + FPSMaster.fontManager.s16.getHeight() + 5 + padding + (index * (itemHeight + padding)), itemWidth - padding, itemHeight, new Color(50, 50, 50, 120));
    ...
    public void renderButton(float x, float y, float mouseX, float mouseY, String icon) {
        ...
        Rects.rounded(x, y, buttonSize, buttonSize, color);
