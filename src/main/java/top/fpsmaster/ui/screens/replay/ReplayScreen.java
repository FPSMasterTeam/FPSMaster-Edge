...
public class ReplayScreen extends ScaledGuiScreen {
    ...
    public void render(int mouseX, int mouseY, float partialTicks) {
        ...
        float panelWidth = Math.min(340f, guiWidth - 24f);
        float panelHeight = Math.min(240f, guiHeight - 32f);
        ...
        FPSMaster.fontManager.getFont(12).drawString(
                String.format(FPSMaster.i18n.get("replay.count.size"), entries.size(), totalBytes / (1024 * 1024)),
                hx + 21f + FPSMaster.fontManager.s16.getStringWidth(FPSMaster.i18n.get("replay.title")) + 5f,
                panelY + 8f,
        ...
        String footText = status.isEmpty() ? FPSMaster.i18n.get("replay.foot") : status;
    ...
    private void drawList(float x, float y, float width, float height, int mouseX, int mouseY) {
        ...
        for (int index = 0; index < entries.size(); index++) {
            ...
            if (hovered) {
                ...
                float playX = renX - (unreadable ? 3f : 5f) - playW;
    ...
    private void drawDialog(int mouseX, int mouseY) {
        ...
        String name = dialogTarget == null ? "" : dialogTarget.displayName();
        if (dialog == Dialog.DELETE) {
            UiChrome.boldString(FPSMaster.fontManager.s14,
                    String.format(FPSMaster.i18n.get("replay.delete.title"), name),
