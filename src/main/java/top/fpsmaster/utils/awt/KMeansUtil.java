...
public class KMeansUtil {
    ...
    // 计算两个颜色点之间的欧几里得距离 (在 RGB 空间中)
    // 参数修改为 List<Integer>
    private static double getEuclideanDistance(List<Integer> pixel1, List<Integer> pixel2) {
        int dr = pixel1.get(0) - pixel2.get(0);
        int dg = pixel1.get(1) - pixel2.get(1);
        int db = pixel1.get(2) - pixel2.get(2);
        return Math.sqrt(dr * dr + dg * dg + db * db);
    }
}
